...
<#
  ...
  Alt remaps, Win+Shift+S -> ShareX) after a game exits. pre-game.ps1 kills it,
  because AHK's low-level keyboard hook is a raw-input liability and an
  anti-cheat red flag.
  ...
  WINDOW AND WORKSPACE HANDLING IS NO LONGER HERE. It moved into the GlazeWM
  Playnite extension (OnGameStarted / OnGameStopped), which is where it belonged:
  no PowerShell, no Python worker, and no global-script wiring for anyone to
  reproduce by hand. This script does one thing now.
#>
param(
    [object] $Game
)
